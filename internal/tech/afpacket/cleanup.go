...
func (c *Context) GetCleanupFunc() error {
	...
	if c.CleanupFunc == nil {
		return fmt.Errorf("failed to retrieve AF_PACKET 'Cleanup' function: ret is nil")
	}
...
func (c *Context) Cleanup(threads int) error {
	...
	if ret != 0 {
		return fmt.Errorf("failed to cleanup AF_PACKET tech: invalid return code (%d)", ret)
	}
